...
create table coches_de_empresa.coches(
...
insert into coches_de_empresa.coches (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales)values
...
create table coches_de_empresa.grupo(
	id serial primary key,
	nombre varchar(70) not null
);

create table coches_de_empresa.moneda(
	id serial primary key,
	nombre varchar(20) not null
);
...
create table coches_de_empresa.poliza(
	id serial primary key,
...
create table coches_de_empresa.aseguradora(
	id serial primary key,
	nombre varchar(30)not null
);
...
alter table coches_de_empresa.poliza add constraint fk_poliza_aseguradora foreign key (id_aseguradora) references coches_de_empresa.aseguradora(id);
alter table coches_de_empresa.poliza add constraint fk_poliza_coche foreign key (matricula) references coches_de_empresa.coche(matricula);

alter table coches_de_empresa.marca  add constraint fk_marca_grupo foreign key (id_grupo) references coches_de_empresa.grupo(id);

alter table coches_de_empresa.modelo add constraint fk_modelo_marca foreign key(id_marca) references coches_de_empresa.marca(id);

alter table coches_de_empresa.coche add constraint fk_coche_modelo foreign key (id_modelo) references coches_de_empresa.modelo(id);
alter table coches_de_empresa.coche add constraint fk_coche_color foreign key (id_color)references coches_de_empresa.color(id);


alter table coches_de_empresa.revision  add constraint fk_revision foreign key (matricula)references coches_de_empresa.coche(matricula);
alter table coches_de_empresa.revision add constraint fk_moneda foreign key (id_moneda) references coches_de_empresa.moneda(id);



insert into coches_de_empresa.grupo (nombre) 
select grupo 
from coches_de_empresa.coches 
group by grupo;


insert into coches_de_empresa.marca(id_grupo, nombre)
select g.id, tc.marca  
from coches_de_empresa.coches tc
inner join coches_de_empresa.grupo g on g.nombre = tc.grupo
group by g.id, tc.marca;

insert into coches_de_empresa.modelo(id_marca, nombre)
select m.id, tc.modelo 
from coches_de_empresa.coches tc
inner join coches_de_empresa.grupo g on g.nombre = tc.grupo 
inner join coches_de_empresa.marca m on m.nombre = tc.marca 
group by g.id, m.id, tc.modelo ;


insert into coches_de_empresa.aseguradora (nombre)
select aseguradora 
from coches_de_empresa.coches 
group by aseguradora;


insert into coches_de_empresa.color (nombre)
select color 
from coches_de_empresa.coches  
group by color;

insert into coches_de_empresa.moneda(nombre)
select moneda 
from coches_de_empresa.coches 
group by moneda;


insert into coches_de_empresa.coche (matricula , fecha_compra, id_modelo, id_color, kms)
select matricula, fecha_compra, mol.id, c.id, kms_totales 
from coches_de_empresa.coches tc
inner join coches_de_empresa.grupo g on g.nombre = tc.grupo 
inner join coches_de_empresa.marca m on m.nombre = tc.marca 
inner join coches_de_empresa.modelo mol on mol.nombre = tc.modelo 
inner join coches_de_empresa.color c on c.nombre  = tc.color 
group by matricula, fecha_compra , mol.id, c.id, kms_totales;


insert into coches_de_empresa.poliza (matricula, id, id_aseguradora, fecha_alta_seguro)
select tc.matricula, tc.n_poliza, a.id, tc.fecha_alta_seguro
from coches_de_empresa.coches tc
inner join coches_de_empresa.aseguradora a on a.nombre  = tc.aseguradora 
group by tc.matricula, tc.n_poliza, a.id, tc.fecha_alta_seguro 
order by tc.matricula, tc.n_poliza;



insert into  coches_de_empresa.revision (matricula, kms, id_moneda, fecha_revision, importe_revision)
select tc.matricula matricula, tc.kms_revision, m.id, tc.fecha_revision, tc.importe_revision 
from coches_de_empresa.coches tc
inner join coches_de_empresa.moneda m on m.nombre = tc.moneda 
group by tc.matricula, tc.kms_revision, m.id, tc.fecha_revision, tc.importe_revision 
order by tc.matricula ;


drop table coches_de_empresa.coches;


select g.nombre , mo.nombre , mar.nombre marca, co.fecha_compra, co.matricula, c.nombre color, co.kms total_kilometros, a.nombre aseguradora, p.id poliza
from coches_de_empresa.coche co
inner join coches_de_empresa.modelo mo on mo.id = co.id_modelo 
inner join coches_de_empresa.marca mar on mar.id = mo.id_marca 
inner join coches_de_empresa.grupo g on g.id = mar.id_grupo
inner join coches_de_empresa.poliza p on p.matricula = co.matricula 
inner join coches_de_empresa.aseguradora a on a.id = p.id_aseguradora
inner join coches_de_empresa.color c on c.id = co.id_color;
